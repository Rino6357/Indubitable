  ...
  return (
    <div className="space-y-8">
      <section className="space-y-4">
        <h1 className="text-3xl font-bold uppercase">Stories</h1>
      ...
      <section className="pixel-box p-4">
        <ul className="space-y-2">
          {stories.map(({ slug, date, title }) => (
            <li key={slug}>
              <Link href={`/writing/stories/${slug}`} className="block hover:bg-foreground hover:text-background px-1 text-sm">
                {date} - {title}.txt
              </Link>
            </li>
          ))}
